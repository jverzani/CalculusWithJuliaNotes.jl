...
\int \frac{Bx+C}{x^2 + bx + c}     &= \frac{B}{2} \log(x^2 + bx + c)\\
&\quad + \frac{2C-bB}{\sqrt{4c-b^2}}\cdot \arctan\left(\frac{2x+b}{\sqrt{4c-b^2}}\right)\\
...
$$
\begin{align*}
\int f(x)/g(x) &= P(x) + \frac{C(x)}{D{x}} + \sum v_i \log(V_i(x))\\
&\quad + \sum w_j \arctan(W_j(x)).
\end{align*}
$$

(Bronstein also sketched the modern method which is to use a Hermite reduction to express $\int (f/g) dx = p/q + \int (g/h) dx$, where $h$ is square free (the "`j`" are all $1$). The latter can be written over the complex numbers as logarithmic terms of the form $\log(x-a)$, the "`a`s"found following a method due to Trager and Lazard, and Rioboo, which is mentioned in the SymPy documentation as the method used.)
